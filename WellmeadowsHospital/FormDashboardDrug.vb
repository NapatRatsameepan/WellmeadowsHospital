...
Public Class FormDashboardDrug
    ...
    Private Sub FormDashboardDrug_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub LoadYears()
        Try
            ...
            Using conn As New SqlConnection(connectionString)
                Dim query As String = "SELECT DISTINCT Order_Year FROM View_DrugExpenses ORDER BY Order_Year"
    ...
    Private Sub LoadMonths(selectedYear As String)
        Try
            ...
            Using conn As New SqlConnection(connectionString)
                Dim query As String = "SELECT DISTINCT Order_Month FROM View_DrugExpenses WHERE Order_Year = @Year ORDER BY Order_Month"
                Using cmd As New SqlCommand(query, conn)
    ...
    Private Sub LoadDataGrid()
        Try
            ...
            Using conn As New SqlConnection(connectionString)
                Dim query As String = "SELECT * FROM View_DrugOrder WHERE Order_Year = @Year AND Order_Month = @Month"
                Using cmd As New SqlCommand(query, conn)
            ...
            For Each row As DataRow In dt.Rows
                totalSum += Convert.ToDecimal(row("Drug_Price_Total")) ' Example column for total price
            Next
    ...
    Private Sub LoadReport()
        Try
            ...
            Using conn As New SqlConnection(connectionString)
                Dim query As String = "SELECT * FROM View_DrugExpenses WHERE Order_Year = @Year"
                Using cmd As New SqlCommand(query, conn)
            ...
            Dim reportPath As String = Application.StartupPath & "\Reports\ReportDrugOrder.rdlc"
